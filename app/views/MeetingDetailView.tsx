...
import { ChevronLeft, Play, Pause, SkipForward, SkipBack, List, Share2, CheckCircle, Clock, Brain, Plus } from 'lucide-react';
    ...
    <div className="flex flex-col h-full animate-in fade-in duration-500 text-zinc-100">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-md transition-all">
            <ChevronLeft size={18} />
          </button>
          <div>
            ...
            <div className="flex items-center gap-4 mt-1">
              ...
              <div className="flex -space-x-2">
                {meeting.participants.map((p, i) => (
                  <div key={i} className="w-6 h-6 rounded-md border border-black bg-zinc-800 overflow-hidden shadow-sm" title={p}>
                    <img src={`https://picsum.photos/seed/${p}/100`} alt={p} className="opacity-80 rounded-md" />
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="lg:w-[65%] flex flex-col gap-6 overflow-y-auto pr-2 scrollbar-hide">
          ...
          <div className="relative group bg-black rounded-md aspect-video w-full overflow-hidden border border-white/10 shadow-2xl flex-shrink-0">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              ...
              >
                {isPlaying ? <Pause size={28} /> : <Play size={28} className="ml-1.5" />}
              </button>
            </div>
          ...
          <div className="glass-card p-6 rounded-md border border-white/5">
            <h3 className="text-[11px] font-bold text-zinc-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
              <Brain size={16} className="text-indigo-400" /> Intelligence report
            </h3>
            <div className="space-y-3">
              {meeting.summary.map((point, i) => (
                <div key={i} className="flex gap-3">
                  <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5 flex-shrink-0"></div>
                  <p className="text-sm text-zinc-300 leading-relaxed font-medium">{point}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="glass-card rounded-md overflow-hidden flex flex-col min-h-[400px] border border-white/5">
            <div className="flex border-b border-white/5 bg-white/[0.02]">
              <button 
                ...
                className={`flex-1 py-4 text-[11px] font-bold uppercase tracking-widest relative transition-colors ${activeTab === 'transcript' ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                Full Transcript
                {activeTab === 'transcript' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-indigo-500"></div>}
              </button>
              <button 
                onClick={() => setActiveTab('mom')}
                className={`flex-1 py-4 text-[11px] font-bold uppercase tracking-widest relative transition-colors ${activeTab === 'mom' ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                Minutes of Meeting
                {activeTab === 'mom' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-indigo-500"></div>}
              </button>
            </div>
              ...
              {activeTab === 'transcript' ? (
                <div className="space-y-8">
                  {meeting.transcript.map((line, i) => (
                    <div key={i} className={`flex gap-5 group ${line.highlight ? 'bg-white/[0.03] -mx-6 px-6 py-4 border-y border-white/5' : ''}`}>
                      <div className="w-10 h-10 rounded-md bg-zinc-900 flex-shrink-0 overflow-hidden border border-white/10">
                        <img src={`https://picsum.photos/seed/${line.speaker}/50`} alt="" className="opacity-80 rounded-md" />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <span className="text-sm font-bold text-zinc-100">{line.speaker}</span>
                          <span className="text-[10px] font-mono text-zinc-500">{line.timestamp}</span>
                        </div>
                        <p className={`text-[13px] leading-relaxed font-medium ${line.highlight ? 'text-zinc-100' : 'text-zinc-400'}`}>
                          {line.text}
                        </p>
                      </div>
                    </div>
                ...
                <div className="space-y-6 text-zinc-300 p-2">
                  <div>
                    <h4 className="text-[11px] font-bold text-indigo-400 uppercase tracking-widest mb-3">Core Outcomes</h4>
                    <ul className="list-disc list-inside space-y-3 text-sm leading-relaxed">
                      <li>Strategic alignment on Q3 vision for engineering velocity.</li>
                      <li>Postponed internal analytics migration to focus on customer-facing dashboards.</li>
                      <li>Quarterly review scheduled for June 22nd.</li>
                    </ul>
                  </div>
                  <div className="p-5 bg-indigo-500/5 border border-indigo-500/10 rounded-md">
                    <h4 className="text-[11px] font-bold text-white uppercase tracking-widest mb-3">Key Decisions</h4>
                    <div className="flex gap-3">
                      <CheckCircle size={18} className="text-emerald-400 mt-0.5 flex-shrink-0" />
                      <p className="text-sm leading-relaxed font-medium">Approved immediate shift from mobile wireframes to full fidelity prototyping to reduce stakeholder feedback loops.</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="lg:w-[35%] flex flex-col h-full gap-4">
          <div className="glass-card rounded-md flex flex-col h-full overflow-hidden border border-white/5">
            <div className="p-5 border-b border-white/5 bg-white/[0.02] flex items-center justify-between">
              ...
              <span className="text-[9px] font-bold bg-indigo-500 text-white px-2 py-0.5 rounded-md uppercase tracking-tighter shadow-indigo-500/20 shadow-lg">AI Detect</span>
            </div>
            <div className="p-5 flex-1 overflow-y-auto space-y-4">
              {meeting.tasks.map((task) => (
                <div key={task.id} className="p-4 rounded-md border border-white/5 bg-white/[0.02] hover:border-white/20 transition-all group">
                  <div className="flex justify-between items-start mb-3">
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-md border uppercase tracking-widest ${
                      task.priority === 'high' ? 'bg-white text-black border-white' : 'bg-zinc-800 text-zinc-400 border-white/5'
                    }`}>
                      {task.priority}
                    </span>
                    <span className="text-[9px] font-bold text-zinc-500 uppercase">#{task.category}</span>
                  </div>
                  <h5 className="text-[13px] font-bold text-zinc-200 mb-4 leading-snug group-hover:text-white transition-colors">{task.title}</h5>
                  <div className="flex items-center justify-between pt-3 border-t border-white/5">
                    <div className="flex items-center gap-2">
                      <img src={`https://picsum.photos/seed/${task.owner}/50`} className="w-6 h-6 rounded-md border border-white/10 opacity-80 group-hover:opacity-100 transition-opacity" alt="" />
                      <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-tighter">{task.owner}</span>
                    </div>
                    <span className="text-[10px] text-zinc-600 font-bold">{task.dueDate}</span>
                  </div>
                </div>
              ))}
      ...
      {showMappingModal && <SpeakerMappingModal onClose={() => setShowMappingModal(false)} />}
    </div>
  );
};
