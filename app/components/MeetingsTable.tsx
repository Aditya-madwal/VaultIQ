       ...
       <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 px-1">
          ...
          <div className="relative group w-full md:w-auto">
              ...
              <input 
                  ...
                  className="w-full md:w-72 pl-9 pr-4 py-2.5 bg-zinc-900/50 border border-zinc-800 rounded-xl text-xs font-bold text-neutral-200 placeholder-neutral-600 focus:outline-none focus:ring-1 focus:ring-green-500/50 focus:border-green-500/30 transition-all uppercase tracking-wide"
              />
          </div>
        </div>

      {filteredMeetings.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 border border-zinc-800/50 rounded-2xl bg-zinc-900/10">
           <Search className="mb-3 text-zinc-700" size={32} strokeWidth={1.5} />
           <p className="text-zinc-500 font-medium text-sm">No meetings found.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="relative z-10 space-y-4">
                 ...
                 <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 px-2 py-1 rounded-md bg-zinc-950/50 border border-zinc-800/50 text-[10px] font-bold text-neutral-400 group-hover:text-neutral-300 transition-colors">
                    ...
                    <span className={`px-2.5 py-0.5 text-[9px] font-black tracking-widest uppercase rounded-full border ${
                        meeting.category === 'Client' ? 'bg-emerald-500/5 text-emerald-500 border-emerald-500/20' :
                        meeting.category === 'Internal' ? 'bg-blue-500/5 text-blue-500 border-blue-500/20' :
                 ...
                 {/* Content */}
                 <div>
                    <h3 className="text-base font-bold text-neutral-100 leading-tight group-hover:text-indigo-200 transition-colors line-clamp-2 min-h-[2.5rem]">
                      {meeting.title}
                    </h3>
              ...
              <div className="relative z-10 mt-5 pt-3 border-t border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-1.5 flex-wrap">
                     {meeting.tags.slice(0, 2).map((tag, i) => (
                        <span key={i} className="text-[9px] px-1.5 py-0.5 rounded bg-zinc-800/50 text-neutral-500 font-bold uppercase tracking-wider group-hover:bg-zinc-800 group-hover:text-neutral-400 transition-colors">
                            #{tag}
                        </span>
                     ))}
                     {meeting.tags.length > 2 && (
                        <span className="text-[9px] text-neutral-600 font-medium">+{meeting.tags.length - 2}</span>
                     )}
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                       ...
                       <div className="p-1.5 text-neutral-600 group-hover:text-indigo-400 transition-colors">
